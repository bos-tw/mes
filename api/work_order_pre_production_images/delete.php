...
$pdo = db();
$tableName = 'work_order_pre_production_images';
$input = getJsonInput();
...
try {
    ...
    $stmt->execute(['id' => $id]);
    logAuditAction('Deleted pre-production work order image', $tableName, $id, ['old_data' => $record]);
    jsonResponse(['success' => true, 'message' => '工單圖片附件已刪除。']);
} catch (Exception $e) {
    error_log('Delete pre-production image failed: ' . $e->getMessage());
