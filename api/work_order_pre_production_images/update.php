...
try {
    ...
    $stmt->execute($params);
    logAuditAction('Updated pre-production work order image', $tableName, $id, $params);
    $updated = findExecutionImageRecord($pdo, $tableName, $id);
    jsonResponse([
        'success' => true,
        'message' => '工單圖片附件資訊更新成功。',
        'data' => $updated ? transformExecutionImageRecord($updated) : null,
    ]);
} catch (Exception $e) {
    error_log('Update pre-production image failed: ' . $e->getMessage());
