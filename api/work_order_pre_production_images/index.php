...
$config = [
    'table' => 'work_order_pre_production_images',
    'label' => '工單圖片附件',
    'upload_dir' => 'uploads/work_order_pre_production_images',
    'file_prefix' => 'wo_pre_production',
    'audit_table' => 'work_order_pre_production_images',
    'success_message' => '工單圖片附件上傳成功。',
    'audit_action' => 'Uploaded pre-production work order image',
    'operation_log_action_key' => 'upload_pre_production_image',
    'operation_log_action_label' => '上傳工單圖片附件',
];

if ($method === 'GET') {
    handleListPreProductionImages($pdo, $config);
    return;
}

handleUploadPreProductionImage($pdo, $config);

function handleListPreProductionImages(PDO $pdo, array $config): void
{
...
function handleUploadPreProductionImage(PDO $pdo, array $config): void
{
    ...
    $countStmt = $pdo->prepare('SELECT COUNT(*) FROM work_order_pre_production_images WHERE work_order_id = :work_order_id AND deleted_at IS NULL');
    $countStmt->execute(['work_order_id' => $workOrderId]);
    if ((int)$countStmt->fetchColumn() >= 3) {
        jsonResponse(['success' => false, 'message' => '工單圖片附件最多上傳 3 張。'], 400);
    }
    ...
    try {
        ...
        error_log('Upload pre-production image failed: ' . $e->getMessage());
