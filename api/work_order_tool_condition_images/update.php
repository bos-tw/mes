...
try {
    ...
    $stmt->execute($params);
    logAuditAction('Updated tool condition work order image', $tableName, $id, $params);
    $updated = findExecutionImageRecord($pdo, $tableName, $id);
    jsonResponse([
        'success' => true,
        'message' => '載具狀況圖片資訊更新成功。',
        'data' => $updated ? transformExecutionImageRecord($updated) : null,
    ]);
} catch (Exception $e) {
    error_log('Update tool condition image failed: ' . $e->getMessage());
