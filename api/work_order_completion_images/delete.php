...
$pdo = db();
$tableName = 'work_order_completion_images';
$input = getJsonInput();
...
try {
    ...
    $stmt->execute(['id' => $id]);
    logAuditAction('Deleted completion work order image', $tableName, $id, ['old_data' => $record]);
    jsonResponse(['success' => true, 'message' => '完工圖片已刪除。']);
} catch (Exception $e) {
    error_log('Delete completion image failed: ' . $e->getMessage());
