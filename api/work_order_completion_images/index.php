...
$config = [
    'table' => 'work_order_completion_images',
    'label' => '工單完工圖片',
    'upload_dir' => 'uploads/work_order_completion_images',
    'file_prefix' => 'wo_completion',
    'audit_table' => 'work_order_completion_images',
    'success_message' => '完工圖片上傳成功。',
    'audit_action' => 'Uploaded completion work order image',
    'operation_log_action_key' => 'upload_completion_image',
    'operation_log_action_label' => '上傳完工圖片',
];

if ($method === 'GET') {
    handleListExecutionImages($pdo, $config);
    return;
}

handleUploadExecutionImage($pdo, $config);

function handleListExecutionImages(PDO $pdo, array $config): void
{
    ...
    $rows = array_map('transformExecutionImageRecord', $stmt->fetchAll(PDO::FETCH_ASSOC) ?: []);
    jsonResponse([
        'success' => true,
        'data' => $rows,
        'pagination' => [
...
function handleUploadExecutionImage(PDO $pdo, array $config): void
{
    ...
    try {
        ...
        logAuditAction('Uploaded completion work order image', $config['audit_table'], $imageId, ['work_order_id' => $workOrderId]);
        ...
        error_log('Upload completion image failed: ' . $e->getMessage());
