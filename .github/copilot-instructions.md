...
5. 禁止以「最小可行版本 / MVP / 先做最小版」作為交付標準；每次實作都必須朝完整功能閉環推進。
6. 可以分批施工，但每一批都必須補齊該範圍內必要的資料模型、API、畫面、追溯、驗證與文件更新，不可交付只有欄位、只有畫面、只有 API 或只有備註的半套功能。
7. 若因風險或範圍需要拆小，必須在對應 plan/todo 清楚標示該批工作屬於哪個完整功能閉環，以及尚未完成的剩餘缺口。
...
任何一段資料只要已銜接到後續流程，就禁止把「刪除」當成普通 CRUD 處理。開發時必須避免造成資料從列表消失、但上游/下游仍保留關聯的「幽靈資料」或追溯斷鏈。

#### A. 刪除語意分類（必做）

所有流程型模組的刪除動作必須先判斷語意，只能落在以下三類：

1. `真正刪除 / 軟刪除`：僅限草稿、未被引用、未進入後續流程的資料。
2. `退回上一步`：資料已產生下一站，但下一站尚未出貨、未異動、未形成不可逆追溯時，應引導使用者回上一站調整狀態。
3. `作廢 / 沖銷 / 退貨`：資料已出貨、已有庫存異動、已有品質檢驗、退貨或報表追溯時，不可刪除，只能走反向流程或作廢流程。

#### B. 後端守門（必做）

1. 流程型 delete API 必須先呼叫共用流程守門，不得只依賴前端 confirm。
2. 目前共用守門實作為：
    - `api/common/workflow_guard.php`
    - `api/workflow_guard/check.php`
3. 新增或修改下列流程模組刪除邏輯時，必須同步更新 workflow guard：
    - `orders`
    - `order_items`
    - `work_orders`
    - `inventory_items`
...
1. 前端刪除前必須先呼叫 `api/workflow_guard/check.php?module={module}&action=delete&id={id}`。
2. 禁止只顯示「確定刪除嗎？」這種無上下文訊息。
3. 若資料已進入後續流程，應優先使用「流程影響確認 modal」，不要只用瀏覽器原生 `confirm()`。
4. modal 必須清楚呈現：
    - 此資料已進入後續流程
    - 目前流程節點與單號
    - 可執行動作
...
6. modal 的按鈕語意必須符合流程狀態，不可提供會造成追溯斷鏈的動作：
    - `退回工單`：適用於庫存由工單產生，且尚未出貨/未異動時。
    - `作廢庫存`：適用於需保留追溯但不再納入有效庫存時。
    - `移除配貨`：適用於庫存已被未出貨出貨單分配時。
    - `建立退貨/沖銷`：適用於已確認出貨後。
    - `取消`：永遠必須存在。
7. modal 只負責引導與觸發合法流程；實際是否允許仍必須以後端 workflow guard 判斷為準。
8. 若短期尚未建立共用 modal 元件，允許先用 `confirm()` 過渡，但訊息仍需列出流程影響；後續應補成標準 modal。
9. 若允許刪除，確認訊息需列出流程影響，例如：

```text
此出貨單尚未確認出貨，可以刪除；系統會釋放已配貨庫存並保留刪除追溯。
...
1. 訂單：
    - 若已有訂單品項、工單、庫存或出貨，禁止直接刪除。
    - 應引導從後段流程依序退回或作廢。
2. 工單：
    - 若已完成或已轉庫存，禁止直接刪除。
    - 若需更正，應退回工單狀態並依提示處理庫存。
3. 庫存：
    - 若由工單產生，禁止在庫存頁直接刪除。
    - 若已配貨、已出貨、被出貨單引用或已有入庫以外異動，禁止刪除。
4. 出貨單：
    - 尚未確認出貨時，可刪除但必須釋放配貨，且出貨單應軟刪，保留品項追溯。
    - 已出貨、已送達、有退貨或品質檢驗時，禁止直接刪除，應走退貨、沖銷或作廢。
5. 退貨單：
    - 已處理完成或已影響庫存時，不可硬刪，應保留追溯。
...
1. 若新增作廢欄位，建議使用：
    ...
    - `void_reason`
    - `void_status`
2. 新增欄位必須透過 migration，且同步更新 `tools/sync-local-schema.ps1`。
3. 任何資料修復 migration 必須可重複執行，不可破壞已出貨、已退貨或已有異動的追溯資料。

#### F. 驗收清單（必做）

修改流程型刪除行為後，至少驗證：

1. 後端直接呼叫 delete API 仍會被 workflow guard 擋住。
2. 前端刪除前會顯示流程影響或阻擋原因。
3. `node --check` 對應 JS 通過。
4. `php -l` 對應 API/helper 通過。
5. `node tools/audit-data-sync.js` 的 `P0` 與 `P1` 為 `0`。
6. 若新增 migration，執行 `powershell -ExecutionPolicy Bypass -File .\tools\sync-local-schema.ps1`。
...
## ⚖️ 工單與篩分報表口徑統一規範（2026-05-23 新增，強制）

### 背景與原則

現場實務可能出現遺失、散落、混料或計數誤差，導致「不良品分布支數合計」不等於真實不良重量換算值。
因此工單與報表統計一律採用「重量優先」口徑，分布支數僅作分類與人工調整依據。

### A. 核心口徑（必做）

1. 工單淨重（order_net_weight_kg）
    - `工單淨重 = 訂單總重(kg) - 載具總重(kg)`
2. 篩分後淨重（actual_net_weight_kg）
    - `篩分後淨重 = 生產紀錄重量合計(kg) - 載具總重(kg)`
3. 不良品重量（defect_weight_kg）
    - `不良品重量 = max(工單淨重 - 篩分後淨重, 0)`
4. 良品支數（good_units）
    - `良品支數 = floor(篩分後淨重 * 1000 / 單重(g))`
5. 不良品支數（defect_units）
    - `不良品支數 = round(不良品重量 * 1000 / 單重(g))`

### B. 分布支數定位（必做）

1. `work_order_screening_defects` 的各項 `defect_quantity` 定位為「分布支數」，用於：
    - 不良類別分布
    - 人工調整與追溯
2. 分布合計不得覆蓋重量換算後的 `defect_units` 統計主值。
3. UI 必須可同時看到：
    - 重量換算不良支數
    - 分布合計支數
    - 差異值（重量換算 - 分布合計）
...
1. 「生產工單編輯」與「篩分檢驗結果報表 summary」必須使用相同重量口徑。
2. 篩分明細表的比例分母使用分布合計（`defect_units_distribution`），避免分類佔比失真。
3. 若差異值非 0，視為可接受實務情況，不應自動覆寫人工分布資料。

### D. 目前對應欄位（2026-05-23）

1. 重量口徑主值：
    - `summary.defect_units`（重量換算）
    - `summary.good_units`（重量換算）
2. 分布口徑輔助值：
    - `summary.defect_units_distribution`（分布合計）
3. 差異追蹤值：
    - 工單編輯側欄 `差異(重量-分布)`

### E. 驗收清單（必做）

1. 同一工單在「編輯工單」與「篩分報表」的 `good_units/defect_units` 主值一致。
